...
class Orchestrator:
    ...
    Coordinates between watchers, Qwen Agent, and the vault.
    """
    ...
    def process_with_qwen(self, action_file: Path) -> bool:
        """
        Process an action file with Qwen Agent.
        
        Note: This uses the qwen-agent Python library.
        For actual AI processing, ensure DASHSCOPE_API_KEY is set.
        ...
        try:
            ...
            if result is not None:
                ...
                # API key not set, create plan and mark for manual processing
                self.logger.warning('DASHSCOPE_API_KEY not set. Task queued for manual processing.')
                self.logger.info('Set API key and re-run, or process manually via Obsidian.')
                return False
    ...
    def _run_qwen_agent(self, prompt: str):
        ...
        # Try DashScope (API key required)
        ...
        # No AI provider available - manual mode
        return None
    
    def _run_ollama(self, prompt: str):
        """
        Run Qwen using Ollama (free, local).
    ...
    def _run_dashscope(self, prompt: str):
        """
        ...
        try:
            ...
            self.logger.warning(f'qwen-agent not available: {e}')
            self.logger.info('Processing in manual mode - review and execute tasks manually.')
            return None
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        '--vault', '-v',
        default=r'C:\Project\Hackathon_0\Digital_AI_Employee_FTE\AI_Employee_Vault',
